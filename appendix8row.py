...
class Appendix8Row:
    ...
    def __repr__(self):
        return (
            "Appendix 8 row - date: %s, domicile: %s,  number: %s, price EUR: %s, price BGN: %s"
            % (
                self.date,
                self.domicile,
                self.number,
                self.price_eur,
                self.price_eur * Decimal(1.95583),
            )
        )
    ...
    def add_transaction(self, transaction: Transaction):
        ...
        if self.date != transaction_date:
            raise ValueError(
                "Invalid transaction date. This is the Appendix 8 row for %s, got a transaction for %s"
                % (self.date, transaction_date)
            )

        if self.domicile != transaction.domicile:
            raise ValueError(
                "Invalid domicile. This is the Appendix 8 row for %s, got a transaction from %s"
                % (self.domicile, transaction.domicile)
            )
        ...
        if transaction.currency != "EUR":
            raise ValueError(
                "Only EUR transactions are supported for Appendix 8. Got a transaction in %s"
